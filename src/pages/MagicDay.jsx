...
function MagicDay() {
  ...
  const fetchEntry = async (d) => {
    ...
    if (!token || !d) return;
    try {
      const res = await axios.get(`${API}/magic/entries/${d}`, {
  ...
  return (
    <div className="homepage">
      ...
      {current ? (
        ...
        // ---------- Gallery Layout (2 rows of 4 => banner, repeat) ----------
        <>
          {Array.from({ length: Math.ceil(allDays.length / 8) }).map((_, groupIndex) => {
            ...
            // split into two rows of up to 4 each
            ...
            return (
              <React.Fragment key={groupIndex}>
                  ...
                  {row1.map((item) => (
                    <figure className="column" key={item.day} style={{ margin: 0 }}>
                      <img
                        src={item.src}
                        alt={item.title}
                        style={{ width: "100%", height: "220px", objectFit: "cover", display: "block",borderRadius:"5%" }}
                      />
                      <figcaption style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0" }}>
                ...
                {/* banner (between groups, not after last) */}
                {groupIndex < Math.ceil(allDays.length / 8) - 1 && (
                  <div style={{ marginTop: "5rem",marginBottom:"5rem", backgroundColor: "#ffe6f0", padding: "2% 2%" }}>
                    <img src="/images/image1.jpeg" alt="banner" style={{ display: "block", margin: "0 auto", maxWidth: "80%" }} />
                  </div>
                )}
              </React.Fragment>
            );
