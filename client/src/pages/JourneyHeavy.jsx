...
export default function JourneyHeavy() {
  ...
  useEffect(() => {
    ...
    api
      .get("/journal/notes", { params: { type: "heavy" } })
      .then((r) => setNotes(r.data || []))
      .catch((e) => {
        console.error("List failed:", e.response?.data || e.message);

        // if token invalid/expired, clear the SAME KEY and redirect
  ...
  const newPage = () => {
    ...
    navigate(`/JourneyHeavy/new?date=${today}`);
  };
    ...
    <div className="homepage">
      ...
      <main className="list-wrap">
        ...
        <div className="notes-grid">
          {notes.map((n) => (
            <NavLink key={n.id} to={`/JourneyHeavy/${n.id}`} className="note-row">
